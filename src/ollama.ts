...
import { ImageBlock, Logger, TextBlock, ToolResultBlock, ToolUseBlock } from '@strands-agents/sdk';
...
export class OllamaModel extends Model<OllamaModelConfig> {
  ...
  private _formatRequest(messages: Message[], options?: StreamOptions) {
    ...
    if (systemPrompt) {
      const content = typeof systemPrompt === 'string' 
        ? systemPrompt 
    ...
    for (const msg of messages) {
      for (const block of msg.content) {
        if (block instanceof TextBlock) {
        ...
        } else if (block instanceof ImageBlock && block.source.type === "imageSourceBytes") {
          ...
          for (const result of block.content) {
            formattedMessages.push({
              role: 'tool',
              content: 'json' in result ? JSON.stringify(result.json) : result.text
            });
          }
        }
      }
